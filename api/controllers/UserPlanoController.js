const UserPlano = require('../models/UserPlanoModel')
...
class PlanoController {
    static async criarPlano(req, res) {
        ...
        try {
            const novoPlano = await UserPlano.create({ nome, valor });
    ...
    static async listarTodosPlanos(req, res) {
        try {
            const planos = await UserPlano.find();
    ...
    static async obterPlanoPorId(req, res) {
        ...
        try {
            const plano = await UserPlano.findById(planoId);

            if (!plano) {
                return res.status(404).json({ error: 'UserPlano não encontrado' });
            }
    ...
    static async atualizarPlano(req, res) {
        ...
        try {
            const plano = await UserPlano.findByIdAndUpdate(
                planoId,
                { nome, valor },
                { new: true }
            );
    ...
    static async deletarPlano(req, res) {
        ...
        try {
            const plano = await UserPlano.findByIdAndDelete(planoId);

            if (!plano) {
                return res.status(404).json({ error: 'UserPlano não encontrado' });
            }

            return res.status(200).json({ message: 'UserPlano excluído com sucesso' });
        } catch (error) {
